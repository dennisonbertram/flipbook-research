...
Fourth live re-check on 2026-04-25 at 08:26 UTC: the about panel still says the visible surface contains no HTML, code, specific links, or fields; text is rendered by the image model as pixels; and live video currently joins a custom optimized video model with the image system. This keeps the project direction unchanged but raises the overfit bar. The next useful proof is not cleaner wiggle, or better rectangular text protection, but target pages that change topic, copy, and illustration while remaining one generated pixel surface.
