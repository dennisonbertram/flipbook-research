...
C98 should make transition quality explicit. The run should save and score target crops at `t=0.25`, `t=0.50`, and `t=0.75`, and the training target should become a deterministic moving-layout transition instead of only endpoint interpolation. This keeps the pure neural-canvas requirement intact while testing the harder Flipbook-like promise: the page should be re-renderable through motion, resize, and layout change, not just crossfaded between two good images.
