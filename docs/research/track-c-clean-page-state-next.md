...
C106 shows that direct L1 over the whole changed region is too broad. It restores naturalist OCR (`0.3922`/`0.4808`) and all runs pass, but transition source-residual gain gets worse for most targets: timeline `0.0709`, reef `0.0759`, orbit `0.0566`, deep-sea `0.0353`, and naturalist `0.1341-0.1529`. C107 should keep the direct term but restrict it to source-only remnant regions, so it targets old-source residue instead of every changed pixel.

C107 is directionally more useful but too strong for naturalist. Source-only direct loss at `0.50` passes timeline (`0.0641` gain), orbit (`0.0467`), reef (`0.0566`), and deep-sea (`0.0213`), but the naturalist variants fail OCR even as `time_power=0.25` improves residual gain to `0.1163`. C108 should keep the source-only restriction and cut direct weight to `0.25` to see whether naturalist can recover OCR without giving back the residual improvement.

C108 narrows the direct-loss bracket. The `0.25` source-only direct weight is strong for orbit (`0.0384`, best so far) and improves naturalist residual to `0.1081-0.1096`, but naturalist still misses OCR just below the gate (`0.3396-0.3434`). Timeline regresses (`0.0728`), reef remains middling (`0.0528`), and deep-sea is acceptable but not best (`0.0114`). C109 should focus on naturalist calibration at direct weights `0.15-0.18`, with an orbit robustness repeat at `0.25`.

C109 keeps the branch useful but raises the overfit warning. Naturalist directsrc `0.18` now passes at both tested timings (`0.3889` OCR / `0.1215` residual for `time_power=0.5`, and `0.3913` / `0.1261` for `time_power=0.25`), while directsrc `0.15/time_power=0.25` improves OCR and residual but misses latency by a hair (`1302.489ms`). Orbit directsrc `0.25` repeats as a pass at seed 5 but lands exactly on the latency edge, and deep-sea directsrc `0.15` is residual-clean but too slow. The next step should not chase another decimal. C110 should validate the mechanism with same-seed controls: naturalist directsrc `0.18` versus no-direct at seed 6, orbit directsrc `0.25` versus no-direct at seed 6, and reef/transit holdout checks for directsrc `0.18`.

C110 validates the naturalist-specific part and rejects source-only direct loss as a broad cleanup default. Naturalist `time_power=0.25/directsrc=0.18` improves the same-seed control on OCR (`0.4808` vs `0.4571`) and transition residual gain (`0.1129` vs `0.1274`) at identical latency. The other pairs are mixed or negative: reef directsrc `0.18` trades OCR for a tiny residual win, transit directsrc `0.18` improves OCR while worsening residual, and orbit directsrc `0.25` improves residual while dropping OCR from `1.0000` to `0.7368`. C111 should pivot back to structure: compare target-canvas blend against an always-visible target-state canvas and a state-split target branch, with naturalist directsrc `0.18` tested only as a candidate combination.

C111 finds the next useful structural lever. Keeping a separate target-state canvas always visible to the decoder beats midpoint blending on all matched no-direct pairs: naturalist `0.4954` OCR / `0.1158` residual vs blend `0.4771` / `0.1448`, reef `0.7500` / `0.0380` vs `0.4783` / `0.0474`, and transit `0.7000` / `0.0421` vs `0.6500` / `0.0513`. The directsrc `0.18` term remains unstable in single-decoder variants, but state-split + direct does pass and gives the best naturalist residual in the wave (`0.0988`) at OCR `0.4348`. C112 should validate `target_canvas=always` across timeline, orbit, deep-sea, and a new naturalist seed before promoting it to the default independent-recomposition recipe.

C112 keeps `target_canvas=always` in contention but shows it needs mode selection. It rescues naturalist OCR on seed 8 (`0.4878` vs blend `0.3366`) and improves deep-sea (`0.8276` OCR, `-0.0042` residual). Timeline also improves residual strongly (`0.0508` vs `0.0749`) while keeping high OCR (`0.9032`), but orbit prefers blend for residual (`0.0447` vs always `0.0607`) with both at OCR `1.0000`. State-split/direct is not stable enough to continue: seed 8 fails OCR. C113 should compare `blend`, `gated`, and `always` target-canvas modes on naturalist and orbit, then check whether `gated` preserves the timeline/deep-sea benefits.

C113 confirms that this branch is useful only if we promote target-specific structure, not a universal target-canvas mode. Naturalist `gated` gets the best OCR (`0.4667`) but fails latency and worsens residual (`0.1527`), while `blend` and `always` both pass with close residuals (`0.1278` and `0.1258`) and lower OCR. Orbit does not need gating: blend and always both keep OCR `1.0000` with cleaner residuals than gated (`0.0416`/`0.0440` vs `0.0608`). Timeline favors always over gated (`0.9677` OCR / `0.0453` residual vs `0.8750` / `0.0703`), while deep-sea favors gated on this seed (`0.8966` / `0.0081` vs `0.8125` / `0.0176`). C114 should therefore validate a small mode policy on fresh seeds instead of widening the parameter sweep: naturalist blend-vs-always, orbit blend-vs-always, deep-sea always-vs-gated, timeline blend-vs-always, plus `always` holdout checks for reef and transit.

C114 marks the overfitting boundary for the current recipe. Naturalist is unstable again: blend fails OCR (`0.3030`) and always only passes OCR at the cost of latency (`1364.786ms`). Orbit blend is the only pass, while always has better residual but misses latency; timeline prefers blend on OCR with no residual cost; deep-sea remains the one healthy case, with both always and gated passing. The attempted reef/transit holdouts are not valid because the labels used `reef-poster` and `transit-map`, which are not registered clean-target variants and fall back to the default fixture. C115 should only correct that fixture mistake with same-seed reef/transit blend-vs-always checks; if those do not reproduce the C111 always benefit, this branch should stop promoting target-canvas mode and move to a different structural idea.

C115 corrects the fixture mistake and shows why the next step should be an ablation, not more policy fitting. Reef supports `always` (`0.6111` OCR / `0.0320` residual vs blend `0.5714` / `0.0477`), but transit supports blend (`0.7000` / `0.0597` vs always `0.6341` / `0.0620`). The mode policy is target-specific and seed-sensitive. C116 should remove source-coordinate features on the existing same-seed controls for naturalist, orbit, timeline, deep-sea, reef, and transit. If no-source-coordinate variants improve residual without destroying OCR, source-coordinate conditioning is probably part of the source-persistence mechanism; if OCR collapses, source-coordinate conditioning is still required and the next structural idea must separate source-phase detail from target-page identity more explicitly.

C116 validates the ablation as useful. No-source-coordinate variants improve or rescue most of the current problem cases: naturalist now passes on both blend/always (`0.4464` and `0.4590` OCR), orbit improves to OCR `0.9333` and residual `0.0533`, deep-sea improves to residuals near or below zero, and timeline/transit pass with better residual despite lower OCR. Reef is mixed: OCR improves strongly (`0.7273`), but residual and latency regress. C117 should use fresh seeds to test whether no-source-coordinate is a robust structural default for naturalist/orbit/deep-sea and a residual-oriented option for timeline/transit, while tracking reef separately.

C117 confirms no-source-coordinate is real, not just a same-seed artifact. It gives passes on naturalist always, orbit, timeline, transit, reef, and both deep-sea modes; only naturalist blend misses OCR. The limitation is residual: naturalist remains high (`0.1543`), orbit/timeline/transit are acceptable but not best-in-family, and reef no-source-coordinate is still worse than the earlier source-coordinate reef residual. C118 should test midpoint-gated source coordinates so the renderer keeps source-space phase near endpoints but suppresses it near the transition midpoint where source persistence appears.

C118 answers the generalization question in two layers. It generalizes for pass stability: all seven runs pass. It does not generalize as a single quality setting: midpoint-gated source coordinates help transit (`0.7000` OCR / `0.0538` residual), deep-sea (`0.8571` / `0.0051`), and timeline OCR (`0.9677`), but hurt orbit OCR (`0.8000` vs C117 no-source `1.0000`) and reef OCR/residual (`0.5116` / `0.0576` vs C117 no-source `0.6667` / `0.0489`). Naturalist remains a tradeoff, with `midscale 0.25` reducing residual to `0.1258` but only barely clearing OCR at `0.3617`. C119 should stop looking for one global source-coordinate setting and validate a conditional recipe on fresh seeds: no-source for naturalist/orbit/reef candidates, midpoint-gated source coordinates for timeline/transit/deep-sea candidates, and one reef full-source check.

C119 confirms that the work is useful but target-conditional. Orbit no-source is a clear promote candidate (`1.0000` OCR, residual `0.0372`). Transit midpoint-gated source coordinates repeat the residual improvement while passing (`0.6829`, residual `0.0539`). Reef rejects no-source as the policy winner: full source coordinates improve OCR to `0.7143` versus no-source `0.5217`, with similar residual (`0.0522` vs `0.0511`). Timeline midpoint-gated source coordinates keep OCR high (`0.9375`) but residual is not stable (`0.0707`), and deep-sea midpoint-gated source coordinates pass but are worse than prior no-source residuals. Naturalist is unresolved: no-source is residual-cleaner (`0.1149`) but fails OCR at `0.3400`, while midscale `0.25` passes OCR at `0.4580` but gives residual `0.1538`. C120 should be a policy-freeze validation, not a new sweep: repeat only these existing branches and use same-seed controls where C119 left an ambiguous comparison.

C120 should be treated as the generalization stop point for this branch. The result is not a universal recipe; it is a conditional one that is now stable enough to document. Naturalist chooses no-source-coordinate on the fresh seed (`0.4138` OCR / `0.1076` residual) while midscale `0.25` fails OCR (`0.3061`). Deep-sea also chooses no-source-coordinate (`0.7857` / `0.0017`). Reef chooses full source-coordinate features (`0.7097` / `0.0455`). Transit chooses midpoint-gated source coordinates (`0.6829` / `0.0482`). Orbit no-source continues to pass (`0.9333` / `0.0578`), though residual is seed-variable. Timeline no-source passes on the same seed (`0.9032`) but is worse than C119 midpoint-gated source coordinates on OCR and residual, so timeline should stay midpoint-gated unless a visual audit says otherwise. The next move should be documentation and visual audit of the frozen policy, not a C121 scalar sweep.

C121 should revise that freeze from "stable enough" to "promising but not routable yet." On three unseen GPT Image 2 illustration families, four of six runs passed and the two failures were near misses, so the mechanism is not merely memorizing the original page fixtures. But policy preference moved: microchip did not need full source-coordinate features, glacier split quality against latency, and mycology reversed toward midpoint-gated source coordinates. Track C should therefore keep the conditional policy family as useful evidence, but Track D should not inherit the C120 hand-written router until fresh generated assets/seeds show stable policy preference.

C122 repeats the same generated assets on fresh seeds and improves the confidence level: all six seed-repeat runs pass, and no-source-coordinate wins OCR on glacier, microchip, and mycology. This means the mechanism is much less likely to be pure fixture overfit. The router remains the weak piece, because C121 and C122 disagree on whether glacier/mycology prefer source-coordinate gating. The next useful loop is fresh generated assets for these or adjacent page families; if no-source-coordinate keeps winning there, promote it as the generated-illustration default and keep source-coordinate gating as a fallback only.
